...
class Normalize(nn.Module):
    def __init__(self, mean, std, eps=1e-9):
        super().__init__()
        self.register_buffer('mean', torch.tensor(mean,dtype=torch.float)[:, None, None])
        self.register_buffer('std', torch.tensor(std,dtype=torch.float)[:, None, None])
        self.eps = eps
